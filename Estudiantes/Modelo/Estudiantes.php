...
class Estudiantes extends conexion{
    ...
    public function update($Id, $Nombre,$Apellido,$Documento,$Correo,$Materia,$Docente,$Promedio,$Fecha){
        $statement = $this->db->prepare("UPDATE estudiantes SET NOMBRE= :Nombre, APELLIDO = :Apellido,
            DOCUMENTO = :Documento, CORREO = :Correo, MATERIA = :Materia, DOCENTE = :Docente,
            PROMEDIO = :Promedio, FECHA_REGISTRO = :Fecha WHERE ID_ESTUDIANTE = :Id");
    ...
    public function search($Search){
        ...
        $statement = this->db->prepare(
            "SELECT * FROM estudiantes WHERE NOMBRE LIKE concat('%', :Search, '%') OR 
            APELLIDO LIKE concat('%', :Search, '%') OR DOCUMENTO LIKE concat('%', :Search, '%')
            OR CORREO LIKE concat('%', :Search, '%') OR MATERIA LIKE concat('%', :Search, '%')
